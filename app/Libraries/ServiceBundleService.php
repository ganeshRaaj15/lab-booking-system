...
class ServiceBundleService
{
    ...
    protected function requirementsFromBundleTable(int $serviceId, int $labId): array
    {
        ...
        $rows = $this->requirementModel
            ->select('service_asset_requirements.*, assets.lab_id, assets.name, assets.asset_code, assets.category, assets.model, assets.status, assets.quantity, assets.total_quantity, assets.image')
    ...
    protected function serializeRequirement(array $row): array
    {
        ...
        $isAvailable = $available >= $required && ! in_array($status, ['maintenance', 'faulty', 'decommissioned'], true);
