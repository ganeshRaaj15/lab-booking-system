    ...
    public function mapForAssets(int $daysAhead = 120): array
    {
        ...
        foreach (array_unique(array_merge(array_keys($predictions), array_keys($forecasts))) as $assetId) {
            ...
            $map[(int) $assetId] = [
                ...
                'bookings_last_30d' => (int) round((float) (($features['booking_count_30d'] ?? $features['bookings_last_30d'] ?? 0.0))),
                'bookings_last_90d' => (int) round((float) (($features['booking_count_90d'] ?? $features['bookings_last_90d'] ?? 0.0))),
                'booking_units_last_90d' => (int) round((float) (($features['booking_hours_90d'] ?? $features['booking_units_last_90d'] ?? 0.0))),
