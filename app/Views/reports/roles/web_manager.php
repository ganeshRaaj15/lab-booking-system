...
// Manager sees all sections except users
$managerGroups = array_values(array_filter(
    $sectionGroups,
    static fn(array $g): bool => ($g['id'] ?? '') !== 'users'
));
...
<div class="rpt-nav-wrap">
    <ul class="nav rpt-nav" id="rptManagerTabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="tab-mgr-overview" data-bs-toggle="tab"
                    data-bs-target="#rpt-panel-mgr-overview" type="button" role="tab"
                    aria-controls="rpt-panel-mgr-overview" aria-selected="true">
                <i class="bi bi-grid-3x3-gap"></i> Overview
            </button>
        </li>
        <?php foreach ($managerGroups as $group): ?>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="tab-mgr-<?= esc($group['id']) ?>"
                    data-bs-toggle="tab"
                    data-bs-target="#rpt-panel-mgr-<?= esc($group['id']) ?>"
                    type="button" role="tab"
                    aria-controls="rpt-panel-mgr-<?= esc($group['id']) ?>"
                    aria-selected="false">
...
<!-- ── Tab Content ────────────────────────────────────────────────────── -->
<div class="tab-content" id="rptManagerTabContent">

    <!-- Overview -->
    <div class="tab-pane fade show active" id="rpt-panel-mgr-overview"
         role="tabpanel" aria-labelledby="tab-mgr-overview">
    ...
    <!-- Section tabs -->
    <?php foreach ($managerGroups as $group): ?>
    <div class="tab-pane fade" id="rpt-panel-mgr-<?= esc($group['id']) ?>"
         role="tabpanel" aria-labelledby="tab-mgr-<?= esc($group['id']) ?>">
